...
为了验证系统的混合检索与解析能力，建议准备 20-50 份 PDF 简历作为测试集。

### 1. 数据来源推荐
*   **开源简历库**：在 GitHub 搜索 `awesome-resume` 或 `简历模板`，可获取高质量的 `.pdf` 样例。
*   **搜索引擎采集**：利用 Google/必应高级指令获取真实 PDF：
    `filetype:pdf "项目经历" "Java" "本科" "简历"`
*   **AI 批量生成 (推荐)**：使用 DeepSeek 或 Kimi 等大模型，输入以下 Prompt 批量生成测试用例：
    > "你是一个资深 Java 开发工程师，请生成 5 份不同风格的求职简历。包含：基本信息、教育背景(985/211/普通)、工作经历、项目经历(涵盖高并发/微服务/中间件等技术栈)。请分别按标准模板格式和非标准(混乱标题)格式输出。"
    > 将输出结果保存为 PDF 或图片，即可测试你的 `OCR Fallback` 和 `Sectionizer` 解析边界。

### 💡 小贴士 (Pro-Tips)
- **OCR 鲁棒性测试**：找几份标准 PDF，打印出来后用手机歪歪扭扭地拍照，转成 PDF 上传，这能极其有效地验证 OCR 引擎与文本清洗模块的健壮性。
- **干扰项注入**：在测试集中故意加入几份“非 Java”或“纯前端”简历，用于验证你的 `Search` 和 `Rerank` 是否能将不相关的候选人准确剔除。
- **环境隔离**：建议在 `application-local.yml` 中配置独立的 `ES 索引前缀`（如 `dev_resume_chunk_`），避免测试数据污染正式生产索引。
